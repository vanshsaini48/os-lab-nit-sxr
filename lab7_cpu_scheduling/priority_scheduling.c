/*
 * Lab 7 - Question 3
 * Priority Scheduling (non-preemptive) with arrival time. Among the
 * processes that have arrived, the one with the highest priority
...
struct Process {
    int pid, arrival, burst, priority, wt, tat, completed;
};
...
int main() {
    ...
    for (i = 0; i < n; i++) {
        ...
        printf("Enter priority of process %d (lower = higher priority): ", i + 1);
        if (scanf("%d", &proc[i].priority) != 1) return 1;
    }
    ...
    while (completed < n) {
        ...
        printf("%d\t%d\t%d\t%d\t\t%d\t%d\n",
               ...
               proc[minIdx].priority, proc[minIdx].wt, proc[minIdx].tat);
    }
